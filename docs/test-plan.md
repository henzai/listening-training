...
- [x] **1-6. GitHub Actions CI ワークフロー** (`.github/workflows/ci.yml`)
  ```yaml
  name: CI
  on:
    push:
...
- [ ] **2-1. テストヘルパー作成**
  - [ ] `vitest.config.worker.ts` — `cloudflareTest` プラグイン導入
  - [ ] `worker/test-env.d.ts` — 型宣言 (`cloudflare:test`, `*.sql?raw`, `Cloudflare.Env`)
  - [ ] `worker/test-helpers.ts` — `cloudflare:workers` の `env` から D1 にスキーマを適用するヘルパー
  - [ ] 既存テスト (`tts.test.ts`, `llm.test.ts`) が cloudflare pool で動作確認

- [ ] **2-2. ルートテスト**
  - [ ] `worker/index.test.ts` — GET /api/v1/health → `{ ok: true }`
